...
-- library functions 
	...
	-- elements 
		...
		function library:init_bindable(cfg, instance, element_type)
			cfg.bind_cfg = {
				name = cfg.name,
				flag = cfg.flag .. "_bind",
				key = "none",
			...
			if cfg.name then 
				...
				cfg.bind_KEYBIND_ELEMENT = KEYBIND_ELEMENT
			end

			function cfg.bind_update_ui()
				if cfg.bind_KEYBIND_ELEMENT then
					cfg.bind_KEYBIND_ELEMENT.Visible = (cfg.bind_cfg.active and cfg.bind_cfg.visible and cfg.bind_cfg.key ~= "none")
					
					local mode = cfg.bind_cfg.mode
					local mode_str = "[ " .. string.upper(string.sub(mode, 1, 1)) .. string.sub(mode, 2) .. " ] "
					local val_str = ""
					if element_type == "slider" and cfg.bind_cfg.value then
						val_str = " (" .. tostring(cfg.bind_cfg.value) .. ")"
					end
					
					cfg.bind_KEYBIND_ELEMENT.Text = mode_str .. (cfg.bind_cfg.name or cfg.name or "") .. val_str .. " - " .. string.lower(tostring(cfg.bind_cfg.key_name or cfg.bind_cfg.key))
				end
			end

			function cfg.bind_set_active(active)
				cfg.bind_cfg.active = active
				flags[cfg.bind_cfg.flag] = {
					mode = cfg.bind_cfg.mode,
					key = cfg.bind_cfg.key,
					active = active,
					value = cfg.bind_cfg.value
				}
				
				if active then
					if element_type == "toggle" then
						if cfg.bind_cfg.mode == "toggle" then
							cfg.set(not cfg.enabled)
						elseif cfg.bind_cfg.mode == "hold" or cfg.bind_cfg.mode == "always" then
							cfg.set(true)
						end
					elseif element_type == "slider" or element_type == "dropdown" then
						if cfg.bind_cfg.value ~= nil then
							cfg.set(cfg.bind_cfg.value)
						end
					elseif element_type == "button" then
						cfg.callback()
					end
				else
					if element_type == "toggle" and cfg.bind_cfg.mode == "hold" then
						cfg.set(false)
					end
				end
			...
			library:connection(uis.InputBegan, function(input, game_event)
				...
				if not game_event and selected_key == cfg.bind_cfg.key and cfg.bind_cfg.key ~= "none" then
					if cfg.bind_cfg.mode == "toggle" then
						cfg.bind_set_active(not cfg.bind_cfg.active)
					elseif cfg.bind_cfg.mode == "hold" then
						cfg.bind_set_active(true)
					end
				end
			end)
			...
			library:connection(uis.InputEnded, function(input, game_event)
				local selected_key = input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode or input.UserInputType
				if not game_event and selected_key == cfg.bind_cfg.key and cfg.bind_cfg.key ~= "none" then
					if cfg.bind_cfg.mode == "hold" then
						cfg.bind_set_active(false)
					end
				end
			end)
			...
			if instance then
				instance.MouseButton2Click:Connect(function()
					library:open_global_bind_menu(cfg, element_type, instance)
				end)
			end
			
			function cfg.bind_set(options)
				if type(options) == "table" then
					cfg.bind_cfg.name = options.name or cfg.bind_cfg.name
					cfg.bind_cfg.flag = options.flag or cfg.bind_cfg.flag
					if options.key then
						local key = options.key
						key = key == Enum.KeyCode.Escape and "none" or key
						cfg.bind_cfg.key = key
						local text = tostring(key) ~= "Enums" and (keys[key] or tostring(key):gsub("Enum.", "")) or nil
						cfg.bind_cfg.key_name = text and (tostring(text):gsub("KeyCode.", ""):gsub("UserInputType.", "")) or "none"
					end
					if options.mode then cfg.bind_cfg.mode = options.mode end
					if options.visible ~= nil then cfg.bind_cfg.visible = options.visible end
					if options.value ~= nil then cfg.bind_cfg.value = options.value end
				end
				cfg.bind_update_ui()
				return cfg
			end
		...
		function library:open_global_bind_menu(cfg, element_type, instance)
			if not library.global_bind_menu then
				local sgui_instance = gethui and gethui():FindFirstChildOfClass("ScreenGui") or game.CoreGui:FindFirstChildOfClass("ScreenGui")
				local menu = library:create("Frame", {
					Parent = library.keybind_list.Parent, -- put it near the keybind list or same sgui
					Name = "global_bind_menu",
					Size = dim2(0, 160, 0, 0),
				...
				library.global_bind_menu = menu
				library:apply_theme(menu, "inline", "BackgroundColor3")
				...
				local top_row = library:create("Frame", {
					Parent = menu, BackgroundTransparency = 1, Size = dim2(1, 0, 0, 20),
					LayoutOrder = 1
				})
				local key_label = library:create("TextLabel", {
					Parent = top_row, Position = dim2(0, 0, 0, 0), Size = dim2(1, -24, 1, 0),
					BackgroundColor3 = themes.preset.outline, BorderColor3 = rgb(0,0,0),
					TextColor3 = themes.preset.text, FontFace = library.font, TextSize = 12,
					Text = "none"
				})
				library:apply_theme(key_label, "outline", "BackgroundColor3")
				
				local bind_btn = library:create("TextButton", {
					Parent = top_row, Position = dim2(1, -20, 0, 0), Size = dim2(0, 20, 1, 0),
					BackgroundColor3 = themes.preset.outline, BorderColor3 = rgb(0,0,0),
					TextColor3 = themes.preset.text, FontFace = library.font, TextSize = 12,
					Text = "..."
				})
				library:apply_theme(bind_btn, "outline", "BackgroundColor3")
				
				local mode_row = library:create("Frame", {
					Parent = menu, BackgroundTransparency = 1, Size = dim2(1, 0, 0, 20),
					LayoutOrder = 2
				})
				local mode_list = library:create("UIListLayout", {
					Parent = mode_row, FillDirection = Enum.FillDirection.Horizontal,
					SortOrder = Enum.SortOrder.LayoutOrder, Padding = dim(0, 4)
				})
				local hold_btn = library:create("TextButton", {
					Parent = mode_row, Size = dim2(0.5, -2, 1, 0), BackgroundColor3 = themes.preset.outline,
					BorderColor3 = rgb(0,0,0), TextColor3 = themes.preset.text, FontFace = library.font,
					TextSize = 12, Text = "hold", AutoButtonColor = false
				})
				library:apply_theme(hold_btn, "outline", "BackgroundColor3")
				local toggle_btn = library:create("TextButton", {
					Parent = mode_row, Size = dim2(0.5, -2, 1, 0), BackgroundColor3 = themes.preset.outline,
					BorderColor3 = rgb(0,0,0), TextColor3 = themes.preset.text, FontFace = library.font,
					TextSize = 12, Text = "toggle", AutoButtonColor = false
				})
				library:apply_theme(toggle_btn, "outline", "BackgroundColor3")

				local val_row = library:create("Frame", {
					Parent = menu, BackgroundTransparency = 1, Size = dim2(1, 0, 0, 20),
					LayoutOrder = 3, Visible = false
				})
				local val_box = library:create("TextBox", {
					Parent = val_row, Size = dim2(1, 0, 1, 0), BackgroundColor3 = themes.preset.outline,
					BorderColor3 = rgb(0,0,0), TextColor3 = themes.preset.text, FontFace = library.font,
					TextSize = 12, PlaceholderText = "bind value...", Text = ""
				})
				library:apply_theme(val_box, "outline", "BackgroundColor3")

				local vis_row = library:create("Frame", {
					Parent = menu, BackgroundTransparency = 1, Size = dim2(1, 0, 0, 20),
					LayoutOrder = 4
				})
				local vis_label = library:create("TextLabel", {
					Parent = vis_row, BackgroundTransparency = 1, Size = dim2(1, -20, 1, 0),
					TextColor3 = themes.preset.text, FontFace = library.font, TextSize = 12,
					Text = "visible", TextXAlignment = Enum.TextXAlignment.Left
				})
				local vis_btn = library:create("TextButton", {
					Parent = vis_row, Position = dim2(1, -12, 0.5, -6), Size = dim2(0, 12, 0, 12),
					BackgroundColor3 = themes.preset.outline, BorderColor3 = rgb(0,0,0), Text = "", AutoButtonColor = false
				})
				library:apply_theme(vis_btn, "outline", "BackgroundColor3")
				local vis_inner = library:create("Frame", {
					Parent = vis_btn, Position = dim2(0, 1, 0, 1), Size = dim2(1, -2, 1, -2),
					BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, Visible = false
				})
				library:apply_theme(vis_inner, "accent", "BackgroundColor3")

				local close_btn = library:create("TextButton", {
					Parent = menu.Parent, BackgroundTransparency = 1, Size = dim2(1, 0, 1, 0),
					ZIndex = 49, Text = "", Visible = false
				})
				
				menu.Close = function()
					menu.Visible = false
					close_btn.Visible = false
					if library.global_bind_connection then
						library.global_bind_connection:Disconnect()
						library.global_bind_connection = nil
					end
				end
				close_btn.MouseButton1Click:Connect(menu.Close)

				library.bind_menu_refs = {
					key_label = key_label, bind_btn = bind_btn, hold_btn = hold_btn, toggle_btn = toggle_btn,
					val_row = val_row, val_box = val_box, vis_inner = vis_inner, vis_btn = vis_btn, close_btn = close_btn
				}
			end

			local menu = library.global_bind_menu
			local refs = library.bind_menu_refs

			if menu.Visible and library.current_bind_cfg == cfg then
				menu.Close()
				return
			end
			
			library.current_bind_cfg = cfg

			refs.key_label.Text = string.lower(tostring(cfg.bind_cfg.key_name or cfg.bind_cfg.key))
			refs.vis_inner.Visible = cfg.bind_cfg.visible
			
			if element_type == "slider" or element_type == "dropdown" or element_type == "textbox" then
				refs.val_row.Visible = true
				refs.val_box.Text = tostring(cfg.bind_cfg.value or "")
			else
				refs.val_row.Visible = false
			end

			menu.Position = dim2(0, instance.AbsolutePosition.X + instance.AbsoluteSize.X + 5, 0, instance.AbsolutePosition.Y)
			local screen_size = workspace.CurrentCamera.ViewportSize
			if menu.AbsolutePosition.X + 160 > screen_size.X then
				menu.Position = dim2(0, instance.AbsolutePosition.X - 165, 0, instance.AbsolutePosition.Y)
			end

			menu.Visible = true
			refs.close_btn.Visible = true

			if refs.bind_conn then refs.bind_conn:Disconnect() end
			if refs.hold_conn then refs.hold_conn:Disconnect() end
			if refs.tog_conn then refs.tog_conn:Disconnect() end
			if refs.val_conn then refs.val_conn:Disconnect() end
			if refs.vis_conn then refs.vis_conn:Disconnect() end

			refs.bind_conn = refs.bind_btn.MouseButton1Click:Connect(function()
				refs.key_label.Text = "..."
				if library.global_bind_connection then library.global_bind_connection:Disconnect() end
				library.global_bind_connection = library:connection(uis.InputBegan, function(input, game_event)
					local selected_key = input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode or input.UserInputType
					cfg.bind_set({key = selected_key})
					refs.key_label.Text = string.lower(tostring(cfg.bind_cfg.key_name or cfg.bind_cfg.key))
					library.global_bind_connection:Disconnect()
					library.global_bind_connection = nil
				end)
			...
			refs.hold_conn = refs.hold_btn.MouseButton1Click:Connect(function() cfg.bind_set({mode = "hold"}) end)
			refs.tog_conn = refs.toggle_btn.MouseButton1Click:Connect(function() cfg.bind_set({mode = "toggle"}) end)

			refs.val_conn = refs.val_box.FocusLost:Connect(function()
				local text = refs.val_box.Text
				if element_type == "slider" then
					local num = tonumber(text)
					if num then cfg.bind_set({value = num}) end
				else
					cfg.bind_set({value = text})
				end
			end)

			refs.vis_conn = refs.vis_btn.MouseButton1Click:Connect(function()
				cfg.bind_set({visible = not cfg.bind_cfg.visible})
				refs.vis_inner.Visible = cfg.bind_cfg.visible
			end)
		end
...
function library:window(properties)
		...
		function library:slider(options)
			...
			library:init_bindable(cfg, slider, "slider")
		...
		function library:toggle(options)
			...
			library:init_bindable(cfg, toggle_holder, "toggle")
		...
		function library:keybind(options)
			if self.bind_set then
				self.bind_set(options)
			end
			return self
		end 
		...
		function library:dropdown(options)
			...
			library:init_bindable(cfg, dropdown_holder, "dropdown")
		...
		function library:textbox(options)
			...
			library:init_bindable(cfg, textbox_holder, "textbox")
		...
		function library:button(options)
			...
			library:init_bindable(cfg, button, "button")
